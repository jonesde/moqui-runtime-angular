...
<#macro screen>
    <#assign currentScreenName = sri.getActiveScreenPathName()>
    <#assign activeScreenDef = sri.getActiveScreenDef()>
    <#-- <#assign activeScreenName = activeScreenDef.getScreenName()> -->
    <#assign allSubscreenItems = activeScreenDef.getSubscreensItemsSorted()>
    <#assign allTransitions = activeScreenDef.getAllTransitions()>

    import {Component, OnInit, Injectable} from 'angular2/core';
    <#-- ROUTER_DIRECTIVES gives us RouterOutlet and RouterLink -->
    import {RouteConfig, ROUTER_DIRECTIVES} from 'angular2/router';
    import {Http, Response, Headers, RequestOptions} from 'angular2/http';
    import {Observable}     from 'rxjs/Observable';

    <#-- iterate subscreens and add Component and Service imports -->
    <#list allSubscreenItems as subscreensItem>
    import {${subscreensItem.name}Component, ${subscreensItem.name}Service} from './${currentScreenName}/${subscreensItem.name}.ts';
    </#list>

    @Injectable()
    export class ${currentScreenName}Service {
        constructor(private http: Http) { }
        <#-- iterating transitions will create a actions() method instead:
        getActionsData() {
            return this.http.get("${sri.buildUrl("actions.json").url}").map(res => res.json())
                    .do(data => console.log(data)).catch(this.handleError);
        }
        -->
        <#-- iterate transitions and add a method for each that calls remote transition and gets the data from the JSON response -->
        <#list allTransitions as transitionItem>
        ${transitionItem.getName()?replace(".", "_")}(parameters: any) : Observable<any> {
            let body = JSON.stringify(parameters); let headers = new Headers({ 'Content-Type': 'application/json' }); let options = new RequestOptions({ headers: headers });
            <#-- use .json extension, to get results as JSON instead of redirect or HTML response -->
            return this.http.post("${sri.buildUrl(transitionItem.getName()).url}.json", body, options).map(res => res.json())
                    .do(data => console.log(data)) <#-- see results in the console; TODO: remove this at some point -->
                    .catch(this.handleError)
        }
        </#list>

        <#-- consider more elaborate error handling, send something to the server? -->
        private handleError(error: Response) {
            console.error(error);
            <#-- TODO: test error handling in JSON responses; uses 'errors' from WebFacadeImpl.sendJsonResponseInternal() -->
            return Observable.throw(error.json().errors || 'Server error');
        }
    }

    @Component({
        <#if componentSelector?has_content>selector: '${componentSelector}',</#if>
        template: `
            <#recurse>
        ...
        `,
        <#-- iterate subscreens and list Services imported -->
        providers:  [<#list allSubscreenItems as subscreensItem>${subscreensItem.name}Service<#if subscreensItem_has_next>,</#if></#list>],
        directives: [ROUTER_DIRECTIVES]
    })
    @RouteConfig([
        <#-- iterate subscreens and create route config for each, set useAsDefault on the default subscreen -->
        <#assign defaultSubscreenName = sri.getActiveScreenDef().getDefaultSubscreensItem()!"">
        <#list allSubscreenItems as subscreensItem>
        { path: '/${subscreensItem.name}/...', name: '${subscreensItem.name}', component: ${subscreensItem.name}Component<#if subscreensItem.name == defaultSubscreenName>, useAsDefault: true</#if> }<#if subscreensItem_has_next>,</#if>
        </#list>
    ])
    export class ${currentScreenName}Component implements OnInit {
        constructor(private _screenService: ${currentScreenName}Service) { }
        errorMessage: string;
        actionsResult: any; <#-- should always be a Map/Object -->
        ngOnInit() { this.getActionsData(); }
        getActionsData() { this._screenService.actions().subscribe(responseData => this.actionsResult = responseData, error => this.errorMessage = <any>error); }
        <#-- iterate transitions and add a method for each that calls remote transition and updates the router to another component (if not '.') -->
        <#list allTransitions as transitionItem><#if transitionItem.getName() != "actions">
        ${transitionItem.getName()?replace(".", "_")}(parameters: any) {
            if (!parameters) { return; }
            this._screenService.${transitionItem.getName()?replace(".", "_")}(parameters).subscribe(responseData => this.handleResponse(responseData), error => this.errorMessage = <any>error);
        }
        </#if></#list>

        <#-- TODO there may be id(s) coming back along with stuff for response conditions, maybe evaluate
            conditional-response on server and send back response info? -->
                    ...
                    <#if urlInstance.isPermitted()>
                        <li class="<#if urlInstance.inCurrentScreenPath>active</#if>">
                            <#if urlInstance.disableLink>
                                <a href="#">
                            <#else>
                            <#-- TODO: how to do links to: Screen/Component, Transition/Service? -->
                            <#-- TODO: how to pass parameters? -->
                                <a [routerLink]="['${subscreensItem.name}']">
                                <#-- ${urlInstance.minimalPathUrlWithParams} -->
                            </#if>
...
<#macro section>
    ...
    <#-- TODO: something for condition and actions, or not support them any more? -->
    <#recurse .node["widgets"][0]>
...
<#macro "section-iterate">
    ...
    <#-- need a totally different approach for this (do iteration in TS template): ${sri.renderSection(.node["@name"])} -->
    <template ngFor #${.node["@entry"]} [ngForOf]="context.${.node["@list"]}">
        <#-- TODO: something for condition and actions, or not support them any more? -->
        <#recurse .node["widgets"][0]>
...
<#macro "container-dialog">
    <#-- TODO had ec.resource.expand(), try to do Groovy (server) to TS (client) expression conversion, or otherwise support html/ts compatible in the same screen? -->
    <#assign buttonText = .node["@button-text"]>
    <#assign divId><@nodeId .node/></#assign>
...
<#-- TODO: need a different approach for this, perhaps get the screen's component class and render it: ${sri.renderIncludeScreen(.node["@location"], .node["@share-scope"]!)} -->
...
<#macro link>
    ...
    <#if linkNode["@entity-name"]?has_content>
        ...
        -->
        <#assign linkText = "Render link with @entity-name not yet implemented">
    <#else>
        <#-- TODO: what to do with text-map?
        <#assign textMap = "">
        <#if linkNode["@text-map"]?has_content><#assign textMap = ec.resource.expression(linkNode["@text-map"], "")!></#if>
        -->
        <#-- TODO: this is one point where we need some sort of Groovy to TS conversion... or some way for screens to still render under html (server) and ts (client)? -->
        <#assign linkText = linkNode["@text"]>
    </#if>
    <#if !linkNode["@encode"]?has_content || linkNode["@encode"] == "true"><#assign linkText = linkText?html></#if>
    <#-- TODO: what to do here in url/etc expressions? string expansion often fails -->
    <#-- TODO: what to do here in with url parameters? in most cases they won't be known now -->
    <#-- TODO: NOTE: we just need a totally different approach to get route path or not and handle parameters somehow like for transition calls -->
...
<#macro linkFormLink linkNode linkFormId linkText urlInstance>
    ...
    <#else>
        ...
        <#if (linkNode["@link-type"]! == "anchor" || linkNode["@link-type"]! == "anchor-button") ||
            ...
            <#-- TODO: need different approach for links if we have a screen route path (and parameters, etc) -->
            <#-- TODO: this is one point where we need some sort of Groovy to TS conversion... or some way for screens to still render under html (server) and ts (client)? -->
            <a <#if linkNode["@condition"]?has_content>*ngIf="${linkNode["@condition"]}" </#if>href="${urlText}"<#if linkFormId?has_content> id="${linkFormId}"</#if><#if linkNode["@target-window"]?has_content> target="${linkNode["@target-window"]}"</#if><#if confirmationMessage?has_content> onclick="return confirm('${confirmationMessage?js_string}')"</#if> class="<#if linkNode["@link-type"]! != "anchor">btn btn-primary btn-sm</#if><#if linkNode["@style"]?has_content> ${ec.resource.expand(linkNode["@style"], "")}</#if>"<#if linkNode["@tooltip"]?has_content> data-toggle="tooltip" title="${ec.resource.expand(linkNode["@tooltip"], "")}"</#if>><#if iconClass?has_content><i class="${iconClass}"></i></#if>
            <#t><#if linkNode["image"]?has_content><#visit linkNode["image"][0]><#else>${linkText}</#if>
            <#t></a>
        <#else>
            <#if linkFormId?has_content>
            <button <#if linkNode["@condition"]?has_content>*ngIf="${linkNode["@condition"]}" </#if>type="submit" form="${linkFormId}"<#if confirmationMessage?has_content> onclick="return confirm('${confirmationMessage?js_string}')"</#if> class="<#if linkNode["@link-type"]! == "hidden-form-link">button-plain<#else>btn btn-primary btn-sm</#if><#if linkNode["@style"]?has_content> ${ec.resource.expand(linkNode["@style"], "")}</#if>"<#if linkNode["@tooltip"]?has_content> data-toggle="tooltip" title="${ec.resource.expand(linkNode["@tooltip"], "")}"</#if>><#if iconClass?has_content><i class="${iconClass}"></i> </#if>
...
<#macro linkFormForm linkNode linkFormId linkText urlInstance>
    ...
    <#else>
        <#if (linkNode["@link-type"]! == "anchor" || linkNode["@link-type"]! == "anchor-button") ||
            ...
            <#-- do nothing -->
        <#else>
            <form <#if linkNode["@condition"]?has_content>*ngIf="${linkNode["@condition"]}" </#if>method="post" action="${urlInstance.url}" name="${linkFormId!""}"<#if linkFormId?has_content> id="${linkFormId}"</#if><#if linkNode["@target-window"]?has_content> target="${linkNode["@target-window"]}"</#if>>
...
<#macro image>
    <#-- TODO: this is one point where we need some sort of Groovy to TS conversion... or some way for screens to still render under html (server) and ts (client)? -->
    <img <#if .node["@condition"]?has_content>*ngIf="${.node["@condition"]}" </#if>src="${sri.makeUrlByType(.node["@url"], .node["@url-type"]!"content", .node, "true").getUrlWithParams()}" alt="${.node["@alt"]!"image"}"<#if .node["@id"]?has_content> id="${.node["@id"]}"</#if><#if .node["@width"]?has_content> width="${.node["@width"]}"</#if><#if .node["@height"]?has_content> height="${.node["@height"]}"</#if><#if .node["@style"]?has_content> class="${ec.resource.expand(.node["@style"], "")}"</#if>/>
</#macro>
<#macro label>
    ...
    -->
    <#-- TODO: this is one point where we need some sort of Groovy to TS conversion... or some way for screens to still render under html (server) and ts (client)? -->
    <#assign labelValue = .node["@text"]>
    <#assign divId><@nodeId .node/></#assign>
    <#if labelValue?trim?has_content>
    <#-- TODO: this is one point where we need some sort of Groovy to TS conversion... or some way for screens to still render under html (server) and ts (client)? -->
    <${labelType}<#if .node["@condition"]?has_content>*ngIf="${.node["@condition"]}" </#if><#if divId?has_content> id="${divId}"</#if><#if .node["@style"]?has_content> class="${ec.resource.expand(.node["@style"], "")}"</#if>><#if !.node["@encode"]?has_content || .node["@encode"] == "true">${labelValue?html?replace("\n", "<br>")}<#else>${labelValue}</#if></${labelType}>
    </#if>
</#macro>
<#macro editable>
